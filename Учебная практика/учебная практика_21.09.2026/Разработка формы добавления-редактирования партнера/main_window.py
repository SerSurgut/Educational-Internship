...
from partner_service import get_partner_types, get_partners_with_discount
...
class MainWindow(tk.Tk):
    def __init__(self, partners, partner_types):
        super().__init__()
        ...
        self.create_partner_list(partners)
    ...
    def create_add_button(self):
        add_button = tk.Button(self, text="Добавить партнера",
                               font=(FONT, 12),
                               command=self.open_partner_edit_window)
        add_button.pack(side="bottom", anchor="e", padx=20, pady=(0, 20))

    def create_partner_list(self, partners):
    ...
    def open_partner_edit_window(self):
...
if __name__ == "__main__":
    ...
    partner_types = get_partner_types(connection)
    connection.close()

    window = MainWindow(partners, partner_types)
    window.mainloop()
